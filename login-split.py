...
conn= pymysql.connect(host='localhost', user='root', password='St3vang@r907', db='Project')
app = Flask(__name__)
...
def display_deals():
    ...
    try:
        c = conn.cursor()
        query = "SELECT * from MaterialIndent"
        c.execute(query)
        data = c.fetchall()
        conn.close()

        return render_template("fancyresults.html", data=data)

    except Exception as e:
        return (str(e))

@app.route('/dropdownsearch/')
def ddsearch():

    try:
        c = conn.cursor()
        queryTable = "SELECT ItemCode,ItemName,ItemDescript,Price,VendorName from Item"
        c.execute(queryTable)
        dataTable = c.fetchall()
        conn.close()

        return render_template("fancysearch2.html", table=dataTable)
